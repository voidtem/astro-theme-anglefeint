...
test('pickDefaultCoverBySlug is deterministic for same slug', () => {
  const covers = [
    '/tmp/covers/ai-01.webp',
    '/tmp/covers/cyber-01.webp',
    '/tmp/covers/matrix-01.webp',
  ];
...
test('buildNewPostTemplate emits expected locale strings', () => {
  const template = buildNewPostTemplate(
    'es',
    'hola-mundo',
    '2026-03-03',
    '../../../assets/blog/default-covers/ai-01.webp'
  );
...
test('buildNewPostTemplate falls back safely for unknown locales', () => {
  const template = buildNewPostTemplate('fr', 'bonjour-monde', '2026-03-03', '');
  assert.match(template, /^title: '.+'/m);
  assert.match(template, /^description: '.+'/m);
  assert.match(template, /^pubDate: '2026-03-03'$/m);
  assert.doesNotMatch(template, /undefined/);
  assert.match(template, /Write your fr content for "bonjour-monde" here\./);
});
